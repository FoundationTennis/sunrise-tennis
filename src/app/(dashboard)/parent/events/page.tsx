...
export default async function ParentEventsPage() {
  ...
  return (
    <div>
      <PageHeader title="Events" description="Club events, socials, and tournaments." />
      ...
      {events && events.length > 0 ? (
        <div className="mt-6 space-y-3">
          {events.map((event) => (
            <div
              key={event.id}
              className="rounded-xl border border-border bg-card p-4 shadow-card transition-colors hover:border-primary/20"
            >
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0 flex-1">
                  ...
                  {event.description && (
                    <p className="mt-2 text-sm text-muted-foreground/80">{event.description}</p>
                  )}
                </div>
                {event.external_url && (
                  <a

    ...
    <div className="overflow-x-auto rounded-lg border border-border bg-card shadow-card">
      <div className="min-w-[700px]">
        ...
        <div className="grid grid-cols-[60px_repeat(7,1fr)] border-b border-border bg-muted/50">
          <div className="p-2" />
          {DAYS.map((day) => (
            <div key={day} className="border-l border-border p-2 text-center text-xs font-semibold text-foreground">
        ...
        <div className="relative grid grid-cols-[60px_repeat(7,1fr)]">
          ...
          {DAYS.map((day, colIdx) => (
            <div key={day} className="relative border-l border-border" style={{ height: visibleHours.length * hourHeight }}>
              {/* Hour grid lines */}
              {visibleHours.map((hour) => (
                <div
                  key={hour}
